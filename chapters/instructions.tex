...
Pictures can be drawn with the \rawref{tikz} package.
To generate the above masterpiece \cref{tikz-example} the following code was used

%Language can be specified as an optional argument.
% A default language is specified in the preamble.
...
The \rawref{out} key describes the angle going \textit{out} of a point,
such as in the example going out of $(0, 0)$
The \rawref{in} key describes the angle going \textit{in} to a point,
such as in the example going in to $(3, 0)$

\subsection{Semantic tableau}
\begin{figure}
    \centering
...
The code for the semantic tableau \cref{semantic-tableau-example} can be seen at \cref{code-semantic-tableau-example}.
Remember the \rawref{=} sign after \rawref{,expand} and to have the correct amount of arguments. 
